...
class RecipeCacheRepository(BaseRedisRepository):
    ...
    async def get_recipe_count(self, user_id: int) -> int | None:
        ...
        raw = await self.redis.get(RedisKeys.recipe_count(user_id=user_id))
    ...
    async def set_recipe_count(self, user_id: int, count: int) -> None:
        ...
        await self.redis.setex(RedisKeys.recipe_count(user_id=user_id), count_ttl, str(count))

    async def invalidate_recipe_count(self, user_id: int) -> None:
        """Удаляет кэш количества рецептов пользователя."""
        await self.redis.delete(RedisKeys.recipe_count(user_id=user_id))
    ...
    async def get_all_recipes_ids_and_titles(self, user_id: int, category_id: int) -> list[dict[str, int | str]] | None:
        ...
        raw = await self.redis.get(RedisKeys.user_recipes_ids_and_titles(user_id, category_id))
    ...
    async def invalidate_all_recipes_ids_and_titles(self, user_id: int, category_id: int) -> None:
        ...
        await self.redis.delete(RedisKeys.user_recipes_ids_and_titles(user_id, category_id))
