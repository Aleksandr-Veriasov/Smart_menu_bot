...
def _recipe_to_dict(r: Recipe) -> dict[str, int | str]:
...
class RecipeService(BaseService):
    ...
    async def get_all_by_user_and_category(self, user_id: int, category_id: int) -> list[dict[str, int | str]]:
        ...
        async with self._lock(RedisKeys.user_init_lock(user_id=user_id)):
            async with self.db.session() as session:
                recipes = await RecipeRepository(session).get_all_by_user_and_category(user_id, category_id)
                rows = [_recipe_to_dict(r) for r in recipes]
    ...
    async def get_public_recipes_ids_and_titles(
        ...
        async with self.db.session() as session:
            recipes = await RecipeRepository(session).get_public_recipes_by_category(
    ...
    async def get_recipe_with_details(self, recipe_id: int) -> Recipe | None:
        ...
        async with self.db.session() as session:
            return await RecipeRepository(session).get_recipe_with_connections(recipe_id)

    async def get_recipe_basic(self, recipe_id: int) -> Recipe | None:
        """Рецепт без связей (название/описание)."""
        async with self.db.session() as session:
            return await RecipeRepository(session).get_by_id(recipe_id)
    ...
    async def find_existing_by_url(self, url: str, user_id: int | None, *, limit: int) -> ExistingRecipeMatch:
        ...
        """
        async with self.db.session() as session:
            videos = await VideoRepository(session).get_all_by_original_url(url, limit=limit)
            if not videos:
            ...
            if user_id:
                already_linked = await RecipeUserRepository(session).is_linked(recipe.id, user_id)
    ...
    async def get_titles_for_ids(self, recipe_ids: list[int]) -> list[tuple[int, str]]:
        ...
        async with self.db.session() as session:
            recipes = await RecipeRepository(session).get_recipes([int(x) for x in recipe_ids])
        id_to_title = {int(r.id): str(r.title) for r in recipes}
    ...
    async def get_recipe_with_link_status(self, recipe_id: int, user_id: int) -> tuple[Recipe | None, bool]:
        ...
        async with self.db.session() as session:
            recipe = await RecipeRepository(session).get_recipe_with_connections(recipe_id)
            if not recipe:
                return None, False
            already_linked = await RecipeUserRepository(session).is_linked(recipe_id, user_id)
    ...
    async def link_recipe_to_user(self, recipe_id: int, user_id: int, category_id: int) -> bool:
        ...
        """
        async with self.db.session() as session:
            created = await RecipeUserRepository(session).upsert_user_link(recipe_id, user_id, category_id)
    ...
    async def search_by_title(self, user_id: int, query: str) -> list[dict[str, int | str]]:
        ...
        async with self.db.session() as session:
            recipes = await RecipeRepository(session).search_recipes_by_title(user_id, query)
        return [_recipe_to_dict(r) for r in recipes]
    ...
    async def search_by_ingredient(self, user_id: int, query: str) -> list[dict[str, int | str]]:
        ...
        async with self.db.session() as session:
            recipes = await RecipeRepository(session).search_recipes_by_ingredient(user_id, query)
        return [_recipe_to_dict(r) for r in recipes]

    async def get_recipe_name(self, recipe_id: int) -> str | None:
        """Название рецепта по id."""
        async with self.db.session() as session:
            return await RecipeRepository(session).get_name_by_id(recipe_id)

    async def delete_recipe(self, user_id: int, recipe_id: int) -> None:
        """Удаляет связь рецепт-пользователь и инвалидирует кэш."""
        async with self.db.session() as session:
            category_id = await RecipeRepository(session).get_category_id_by_recipe_id(recipe_id, user_id)
            logger.debug(f"👉 Рецепт {recipe_id} category_id: {category_id}")
            await RecipeUserRepository(session).unlink_user(recipe_id, user_id)
    ...
    async def save_recipe_draft(
        ...
        *,
        ...
        async with self.db.session() as session:
            recipe = await RecipeRepository(session).create_basic(
                title=title,
            ...
            await RecipeIngredientRepository(session).bulk_link(int(recipe.id), id_by_name.values())
            if video_url:
                await VideoRepository(session).create(video_url, int(recipe.id), original_url=original_url)
